import type { BundleReport, BundleChunk, DiffRow, BundleDiffResult } from './types';
 ...
 */
function chunkKey(c: BundleChunk): string {
...
export function computeDiff(
  ...
  const baselineMap = new Map<string, BundleChunk>();
  for (const c of baseline.chunks) {
    baselineMap.set(chunkKey(c), c);
  }

  const currentMap = new Map<string, BundleChunk>();
  ...
  const prefix = (r: DiffRow) => {
    if (r.changeType === 'added') return `+ ${r.label}`;
    if (r.changeType === 'removed') return `- ${r.label}`;
    return r.label;
  };
